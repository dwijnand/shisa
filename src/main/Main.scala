...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def takeClss(compiler: Compiler, clss: List[Stat]) = doCompile2(compiler, "foo.scala", source"package foo; ..$clss".syntax + "\n")

    val futures = clss.grouped(1000).toList.flatMap { clss =>
      val compiler = SC3.mkCompiler()
      clss.grouped(100).toList.map { clss =>
        pool.submit[List[Msg]](() => takeClss(compiler, clss))
      }
    }
    pool.shutdown()
