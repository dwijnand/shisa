...
object Shareables:
  ...
  object Shareable:
    ...
    given [F[_]: Alternative]: Alternative[[A] =>> Shareable[F, A]] with
      ...
      def pure[A](a: A): Shareable[F, A] = Shareable(_ => Alternative[F].pure(a))
      def empty[A]: Shareable[F, A]      = Shareable(_ => Alternative[F].empty[A])
